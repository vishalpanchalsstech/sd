...
class driverregister extends CI_Controller
{
    ...
    public function insert_registration()
    {
        ...
        {
            // echo "No";exit;
            $token = $this->VersatileAccessToken();
            $user_token = $this->VersatileAccessToken("SST","driver");
            //echo 'yes'.$token.$user_token;
            //exit;
            $to = $email;
            $from = '';
            $cc = '';
            ...
            if($mail_sent == '1'){
                ...
                if($user_query)
                {
                    ...
                    if($driver_query)
                    {
                        ...
                        $msg='<div class="alert alert-success" role="alert"><strong>Registration successfully!! Please check your email for verification.</strong></div>';
                    }
                    else
                    {
